...
const styles = `
  ...
  .static-noise {
    background: repeating-radial-gradient(#111 0 0.0001%, #222 0 0.0002%) 50% 0/2500px 2500px,
    repeating-conic-gradient(#111 0 0.0001%, #222 0 0.0002%) 60% 60%/2500px 2500px;
...
const App = () => {
  ...
  // Ref untuk menyimpan mode terakhir tanpa re-render loop
  ...
  useEffect(() => {
    if (activeTab === 'dashboard') logsEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [logs, activeTab]);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
        try {
            ...
            if (data.type === 'PREFLIGHT_REPORT') {
                setPreflightData(data.report);
                addLog('INFO', 'Pre-flight check completed.');
                return;
            }
            
            // Simpan mode ke Ref agar bisa diakses oleh event listener keyboard
  ...
  const sendCommand = (payload: any) => {
      // FIX: Cek readyState agar tidak error saat koneksi putus
      if(wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
          wsRef.current.send(JSON.stringify(payload));
      } else {
          // Silent fail or log once
          // addLog('ERR', 'No Connection'); 
      }
  };

  const handleArm = () => {
      if (!telemetry.armed && telemetry.mode.includes("STABILIZE")) addLog('WARN', 'Mode is STABILIZE. Switch to HOLD/LOITER first!');
      sendCommand({ type: 'COMMAND_LONG', command: 'MAV_CMD_COMPONENT_ARM_DISARM', param1: telemetry.armed ? 0 : 1 });
  };
  const handleModeChange = (newMode: string) => {
      sendCommand({ type: 'SET_MODE', mode: newMode });
  };
  const runPreflightCheck = () => {
      addLog('CMD', 'Running Diagnostics...');
      setPreflightData(prev => prev.map(p => ({...p, status: 'PENDING', detail: 'Checking...'})));
      sendCommand({ type: 'REQ_PREFLIGHT' });
  };
  
  const uploadFigure8 = () => {
      addLog('CMD', 'Uploading Mission...');
      sendCommand({ type: 'UPLOAD_MISSION_FIGURE8' });
  };
  
  const startMission = () => {
      addLog('CMD', 'Sending START Command...');
      sendCommand({ type: 'SET_MODE', mode: 'MISSION' });
  };
  ...
  useEffect(() => {
    ...
    if (!document.getElementById('leaflet-js')) {
        const script = document.createElement("script"); script.id = 'leaflet-js'; script.src = "https://unpkg.com/leaflet@1.7.1/dist/leaflet.js"; script.async = true; script.onload = () => initMapInstance(); document.head.appendChild(script);
    } else { setTimeout(initMapInstance, 100); }
  ...
  const initMapInstance = () => {
    ...
    const marker = L.marker([startLat, startLng], { icon: droneIcon });
    marker.addTo(map);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // FIX 1: Strict Check - Hanya aktif jika WebSocket OPEN
      if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return;
      
      // FIX 2: Mode Check - Hanya kirim manual control jika mode OFFBOARD
      // Menggunakan Ref agar selalu dapat nilai terbaru tanpa re-bind listener
      ...
      let x = 0, y = 0, z = 0, r = 0;
      if (e.key === 'w' || e.key === 'W') x = SPEED;    
      if (e.key === 's' || e.key === 'S') x = -SPEED;   
      if (e.key === 'a' || e.key === 'A') y = -SPEED;   
      if (e.key === 'd' || e.key === 'D') y = SPEED;    
      ...
      if (x !== 0 || y !== 0 || z !== 0 || r !== 0) {
          wsRef.current.send(JSON.stringify({ type: 'MANUAL_CONTROL', x, y, z, r }));
      }
    };
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-gray-100 font-sans overflow-hidden">
      ...
      <nav className="w-20 bg-gray-800 border-r border-gray-700 flex flex-col items-center py-6 gap-6 z-30 shadow-xl">
         ...
         <div className="text-[10px] text-gray-500 font-mono rotate-180" style={{writingMode: 'vertical-rl'}}>SKYNET GCS v2.4</div>
      </nav>
          ...
          <main className="flex-1 flex overflow-hidden relative">
                    ...
                    <div className="flex-1 relative bg-gray-900">
                        ...
                        {viewMode === 'camera' && (
                            <div className="w-full h-full flex flex-col items-center justify-center relative overflow-hidden bg-black">
                                <div className="absolute inset-0 static-noise opacity-10"></div>
                                <div className="z-10 text-center"><Video size={48} className="text-gray-600 mx-auto mb-2" /><h3 className="text-gray-500 font-mono text-lg">NO VIDEO SIGNAL</h3><p className="text-gray-700 text-xs mt-1">Check VTX / RTSP Connection</p></div>
                                <div className="absolute inset-4 border-2 border-white/20 rounded-lg pointer-events-none flex flex-col justify-between p-4">
                                    <div className="flex justify-between text-green-400 font-mono text-sm shadow-black drop-shadow-md"><span>REC [00:00:00]</span><span>BAT: {telemetry.battery_voltage.toFixed(1)}V</span></div>
                                    ...
                                    <div className="flex justify-between text-green-400 font-mono text-sm shadow-black drop-shadow-md"><span>H: {telemetry.altitude_relative.toFixed(1)}m</span><span>S: {telemetry.ground_speed.toFixed(1)}m/s</span></div>
                                </div>
                            </div>
                        )}
                        {viewMode === 'map' && (
                            ...
                            <div className="space-y-4">
                                <div className="bg-gray-700/50 p-4 rounded-lg flex items-center justify-between border border-gray-600">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="font-bold text-white">Figure 8 (Infinite)</div>
                                            <div className="text-xs text-gray-400">Pattern: 8 • Alt: 15m • Auto Land</div>
                                        </div>
                                    </div>
                                ...
                                <div className="p-4 bg-yellow-900/20 border border-yellow-700/30 rounded-lg text-xs text-yellow-200">
                                    <strong>How to run:</strong> <br/>
                                    1. <strong>Arm</strong> drone first.<br/>
                                    2. Click <strong>UPLOAD</strong> to send waypoints.<br/>
                                    3. Click <strong>START</strong> to fly mission.<br/>
                                    Drone will fly Figure 8 pattern then land at origin.
                                </div>
                            </div>
                        </div>
                        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 flex flex-col items-center justify-center min-h-[300px]">
                            <MapIcon size={48} className="text-gray-600 mb-4 opacity-50"/>
                            <p className="text-gray-500">Mission Map Preview</p>
                            <p className="text-xs text-gray-600">(Map view is on Dashboard)</p>
                        </div>
                    </div>
                 </div>
             )}
             ...
             {activeTab === 'preflight' && (
                 <div className="flex-1 bg-gray-900 p-8 overflow-y-auto">
                            ...
                            <div key={item.id} className="flex items-center justify-between p-5 border-b border-gray-700 last:border-0 hover:bg-gray-750">
                                <div className="flex items-center gap-4">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                        ...
                                        item.status === 'WARN' ? 'bg-yellow-500/20 text-yellow-500' :
                                        'bg-gray-700 text-gray-500'
                                    }`}>
                                        {item.status === 'PASS' ? <CheckCircle size={18}/> :
                                         item.status === 'FAIL' ? <XCircle size={18}/> :
                                         item.status === 'WARN' ? <AlertTriangle size={18}/> :
                                         <Activity size={18}/>}
                                    </div>
                                    <div>
                                        <span className="text-gray-200 text-lg block">{item.label}</span>
                                ...
                                <div className={`px-3 py-1 rounded text-xs font-bold ${
                                    ...
                                    item.status === 'WARN' ? 'bg-yellow-900 text-yellow-300' :
                    ...
                    <div className="mt-6 flex justify-end max-w-3xl">
                        <button 
                            onClick={runPreflightCheck}
                            className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold shadow-lg flex items-center gap-2"
                        >
                            <RefreshCw size={20} className={preflightData.some(i => i.status === 'PENDING' && i.detail === 'Checking...') ? 'animate-spin' : ''}/> 
                            RUN DIAGNOSTICS
                        </button>
                    </div>
                 </div>
